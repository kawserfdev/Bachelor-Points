...
if [[ -f package.json ]]; then
  echo "    Found package.json. Installing npm packages and building Next.js project..."
  ...
  cp -R out/. "${OUTPUT_DIR}/"
  echo "    Landing page built and exported to ${OUTPUT_DIR}"
elif [[ -d out ]]; then
  echo "    package.json not found, but 'out/' directory exists. Copying pre-built static files..."
  cp -R out/. "${OUTPUT_DIR}/"
  echo "    Pre-built static landing page copied to ${OUTPUT_DIR}"
else
  echo "    ERROR: Neither package.json nor pre-built 'out/' directory found in ${LANDING_DIR}!" >&2
  exit 1
fi
